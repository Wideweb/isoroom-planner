...
import { IsTokenExpired, LoadMe, LoginWithGoogle, Logout, RefreshAccessToken, UpdateToken, AuthUserChanged } from "./auth.actions";
...
export class AuthState {
  ...
  @Selector()
  static inited(state: AuthStateModel): boolean {
    return !!state.inited;
  }
  ...
  @Selector()
  static isAuthenticatedNotTemp(state: AuthStateModel): boolean {
    return !!state.refreshToken.value && !state.user?.tempId;
  }
  ...
  async loginWithGoogle(ctx: StateContext<AuthStateModel>, action: LoginWithGoogle) {
    ...
    try 
    {
      ...
      const tempId = ctx.getState().user?.tempId || null;
      const sesionData = await firstValueFrom(this.authService.loginWithGoogle(action.token, ref, tempId));
      ctx.setState(patch<AuthStateModel>({ 
       ...
       this.store.dispatch(new AuthUserChanged());
    } 
    catch(signUpError)
    {
  ...
  logout(ctx: StateContext<AuthStateModel>) {
    ...
    if (accessTokenExpired && refreshTokenExpired) {
      ctx.setState({...defaults(), inited: true});
      return;
    }

    return this.authService.logout().pipe(
      tap(() => this.socialAuthService.signOut()),
      tap(() => ctx.setState({...defaults(), inited: true})),
      catchError(() => {
        ctx.patchState({...defaults(), inited: true});
        return of(null);
      }),
    );
  }
