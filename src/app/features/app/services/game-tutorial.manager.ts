...
export enum TutorialStepID {
    ...
    Moves = 13,
    Replenish = 14,
    ReplenishCategoryCompleted = 15,
    LevelAndProgression = 16,
    End = 17
}
...
export class GameTutorialManager {
    ...
    private TUTORIAL_STEPS: TutorialStep[] = [
        ...
        {
            id: TutorialStepID.FurnitureAccessibility,
            wait: () => waitForElementInZone(this.zone, '.game-board'),
            area: () => this.component.game.getSelectedFurnitureAccessibilityBoundingClientRect(),
            cursorStart: () => null,
            cursorEnd: () => null,
            isCompleted: (event: GameEvent) => false,
            text: 'Each item has accessibility cells that must stay open and reachable from the room\'s entrance.',
            position: 'top-center',
            cursor: 'none',
            action: 'popup',
            blockArea: 'window',
